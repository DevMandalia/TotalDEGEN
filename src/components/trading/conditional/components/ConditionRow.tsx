...
const ConditionRow = ({
  ...
  return (
    <div className="space-y-3">
      {/* Condition Row - Minimized width */}
      <div className="flex items-center gap-2 backdrop-blur-sm bg-white/5 border border-white/10 rounded-lg p-2 justify-center group max-w-2xl mx-auto">
        {/* Open Parenthesis */}
        <button
        ...
        <Dialog open={assetSearchOpen} onOpenChange={setAssetSearchOpen}>
          <DialogTrigger asChild>
            <Button variant="outline" className="backdrop-blur-sm bg-white/10 border border-white/20 text-white hover:bg-white/20 w-32 justify-start text-sm">
        ...
        <Select value={condition.operator} onValueChange={(value) => onUpdate(condition.id, 'operator', value)}>
          <SelectTrigger className="w-12 backdrop-blur-sm bg-white/10 border border-white/20 text-white">
            <SelectValue />
          </SelectTrigger>
        ...
        <Input
          ...
          className="backdrop-blur-sm bg-white/10 border border-white/20 text-white placeholder:text-gray-400 w-24"
        />
        ...
        <Button
          ...
          className="hover:bg-red-500/20 text-red-400 w-8 h-8"
        >
          <X className="w-4 h-4" />
        </Button>
      </div>

      {/* AND/OR Connectors - Moved below condition row */}
      {index < totalConditions - 1 && (
        <div className="flex justify-center gap-3">
          <ToggleGroup type="single" value={condition.connector} onValueChange={(value) => value && onUpdate(condition.id, 'connector', value)}>
            <ToggleGroupItem 
              value="and" 
              className="backdrop-blur-sm bg-gradient-to-r from-emerald-500/30 to-teal-500/30 border border-emerald-400/50 text-emerald-200 data-[state=on]:bg-gradient-to-r data-[state=on]:from-emerald-500/60 data-[state=on]:to-teal-500/60 data-[state=on]:border-emerald-400/80 data-[state=on]:text-emerald-100 px-4 py-1 text-sm"
            >
              AND
            </ToggleGroupItem>
            <ToggleGroupItem 
              value="or" 
              className="backdrop-blur-sm bg-gradient-to-r from-amber-500/30 to-orange-500/30 border border-amber-400/50 text-amber-200 data-[state=on]:bg-gradient-to-r data-[state=on]:from-amber-500/60 data-[state=on]:to-orange-500/60 data-[state=on]:border-amber-400/80 data-[state=on]:text-amber-100 px-4 py-1 text-sm"
            >
              OR
            </ToggleGroupItem>
          </ToggleGroup>
      ...
      {/* Add Condition Button - Only show after last condition if it has a connector */}
      {index === totalConditions - 1 && condition.connector && (
